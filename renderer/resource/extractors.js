    ...
    function getForm(f) {
        ...
        for (; input = f.elements[j]; j++) {
            ...
            // <input type!=reset|button>, <textarea>, <keygen> element
                ...
                if (typeof(f[input.name].length) === 'undefined') // an unique element with such a 'name'
                    values.push([name, value].join('='));
                else if (!dataType[name]) // i.e., radio/checkbox, and the first time being recorded
                    multiDefaults[name] = [].map.call(f[input.name], function(opt){return [name, encodeURIComponent(opt.value)].join('=')});
